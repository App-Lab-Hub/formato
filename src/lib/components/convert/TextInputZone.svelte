...
<div class="w-full max-w-4xl mx-auto">
  <div class="border-2 border-dashed border-muted rounded-2xl p-6 bg-card/30 hover:border-primary/50 transition-colors duration-300">
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center gap-3">
        <div class="p-2 rounded-lg bg-primary/10 text-primary" aria-hidden="true">
          <Type class="h-5 w-5" />
        </div>
        ...
        <div class="flex items-center bg-background border border-muted rounded-lg px-3 py-1.5 focus-within:ring-2 focus-within:ring-primary transition-shadow">
          ...
          />
          <span class="text-sm text-muted-foreground ml-1" aria-hidden="true">.{sourceFormatId}</span>
        </div>
      </div>
      
      <div class="flex items-center gap-2 text-xs text-muted-foreground bg-background/50 px-3 py-1 rounded-full" aria-hidden="true">
    ...
    <div class="relative">
      ...
      <textarea
        ...
        spellcheck="false"
        aria-label={m.enter_text_prompt({ format: sourceFormatName })}
      ></textarea>
    </div>
    
    {#if error}
      <div 
        class="mt-3 text-sm text-destructive bg-destructive/10 px-4 py-2 rounded-lg border border-destructive/20 flex items-center gap-2"
        role="alert"
        aria-live="polite"
      >
  ...
  <div class="mt-4 flex justify-end gap-3">
    <button
      ...
      class="cursor-pointer px-4 py-2 bg-muted text-muted-foreground rounded-lg hover:bg-muted/80 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
      aria-label={m.clear_text()}
    >
    ...
    <button
      ...
      aria-label={isProcessing ? m.adding_text() : m.add_to_list()}
    >
      {#if isProcessing}
