<script lang="ts">
  ...
  async function fadeOut() {
    ...
    if (refs.title) targets.push(animate(refs.title as HTMLElement, { opacity: 0 }, FADE));
  ...
  onMount(() => {
    ...
    async function runSequence() {
      ...
      await animate(refs.subtitle as HTMLElement, 
...
{#if visible}
  <div bind:this={refs.container} class="splash-container">
    ...
    <div bind:this={refs.splashContent} class="splash-content">
      ...
      <h1 bind:this={refs.title} class="splash-title gradient-text">
        Formato
      </h1>
