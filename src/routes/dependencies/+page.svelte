  ...
  <div class="flex flex-col bg-background text-foreground min-h-screen">
    <main class="flex flex-col items-center px-8 py-16 w-full">
      ...
      <div class="w-full max-w-[1700px] flex justify-start mb-8">
        <button onclick={goBack} class="cursor-pointer flex items-center gap-2 dark:text-muted-foreground light:text-muted-foreground/70 dark:hover:text-primary light:hover:text-primary transition-colors">
        ...
        <div class="max-w-4xl mx-auto space-y-6">
          {#if data.deps}
            {#if getNpmGroups(data.deps).length > 0}
              <div class="dark:bg-card/50 light:bg-white/60 backdrop-blur-sm rounded-2xl border dark:border-border light:border-border/60 overflow-hidden">
                <button
                  onclick={toggleNpm}
                  class="w-full flex items-center justify-between p-6 dark:hover:bg-card/30 light:hover:bg-white/40 transition-colors cursor-pointer"
                >
                  <div class="flex items-center gap-3">
                    <Package class="h-5 w-5 text-yellow-400" />
                    <h2 class="text-lg font-semibold dark:text-foreground light:text-foreground/90">{m.deps_npm()}</h2>
                    <span class="text-xs dark:text-muted-foreground/60 light:text-muted-foreground/60 dark:bg-muted-foreground/10 light:bg-muted-foreground/10 px-2 py-1 rounded-full">
                      {getNpmGroups(data.deps).reduce((acc, g) => acc + g.data.length, 0)} {m.deps_packages()}
                    </span>
                  </div>
                  <ChevronDown class="h-5 w-5 dark:text-muted-foreground light:text-muted-foreground/60 transition-transform duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)]" style="transform: rotate({npmOpen ? 180 : 0}deg)" />
                </button>
                
                {#if npmOpen}
                  <div 
                    class="dark:border-t border-border light:border-t border-border/50 p-6 space-y-6" 
                  ...
                  >                
                    {#each getNpmGroups(data.deps) as group}
                      <div>
                        <h3 class="text-sm font-medium dark:text-muted-foreground light:text-muted-foreground/70 mb-3">
                          {group.label}
                          <span class="text-xs dark:text-muted-foreground/40 light:text-muted-foreground/40 ml-1">({group.data.length})</span>
                        </h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                          {#each group.data as dep}
                            <div class="flex items-center justify-between px-4 py-2 dark:bg-background/50 light:bg-white/40 rounded-lg border dark:border-border/50 light:border-border/40 dark:hover:border-primary/30 light:hover:border-primary/40 dark:hover:bg-background/80 light:hover:bg-white/60 transition-all duration-200">
                              <span class="text-sm font-mono dark:text-foreground light:text-foreground/90">{dep.name}</span>
                              <span class="text-xs dark:text-muted-foreground/60 light:text-muted-foreground/50 font-mono">{dep.version}</span>
                            </div>
                          {/each}
                        </div>
                      </div>
              ...
              <div class="dark:bg-card/50 light:bg-white/60 backdrop-blur-sm rounded-2xl border dark:border-border light:border-border/60 overflow-hidden">
                ...
                >
                  <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-lg font-semibold dark:text-foreground light:text-foreground/90">{m.deps_cargo()}</h2>
                    <span class="text-xs dark:text-muted-foreground/60 light:text-muted-foreground/60 dark:bg-muted-foreground/10 light:bg-muted-foreground/10 px-2 py-1 rounded-full">
                      {getCargoGroups(data.deps).reduce((acc, g) => acc + g.data.length, 0)} {m.deps_packages()}
                    </span>
                  </div>
                  <ChevronDown class="h-5 w-5 dark:text-muted-foreground light:text-muted-foreground/60 transition-transform duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)]" style="transform: rotate({cargoOpen ? 180 : 0}deg)" />
                </button>
            ...
            <div class="dark:bg-card/30 light:bg-white/40 backdrop-blur-sm rounded-2xl border dark:border-border light:border-border/60 p-6 dark:hover:border-primary/20 light:hover:border-primary/30 transition-all duration-300">
              <div class="flex items-center gap-3">
                <BookOpen class="h-5 w-5 text-primary" />
                <div>
                  <h3 class="text-sm font-semibold dark:text-foreground light:text-foreground/90">{m.deps_where()}</h3>
                  <p class="text-xs dark:text-muted-foreground/60 light:text-muted-foreground/60 mt-1">
                    {m.deps_where_desc()}
                  </p>
                </div>
              </div>
            ...
            <div class="text-center text-xs dark:text-muted-foreground/40 light:text-muted-foreground/40 pb-8">
