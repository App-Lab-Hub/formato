...
pub fn stringify_odt(text: &str, path: &str, from: &str, to: &str) -> Result<String, String> {
    ...
    save_odt(&output_path, &document).map_err(|e| format!("ODT generation error: {:?}", e))?;
...
pub fn parse_odt(path: &str) -> Result<Json, String> {
    ...
    let mut archive = ZipArchive::new(file).map_err(|e| format!("ODT parse error: {}", e))?;
                    ...
                    "draw:image" => {
                        for attr in e.attributes().flatten() {
                            ...
                            if attr_name == "xlink:href" {
                                if let Ok(href) = attr.normalized_value(XmlVersion::Implicit1_0) {
                ...
                match name.as_ref() {
                    ...
                    "text:list" => {
                        if !list_items.is_empty() {
                            let list_json: Vec<Json> =
                                list_items.iter().map(|s| Json::String(s.clone())).collect();
