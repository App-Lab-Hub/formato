...
pub async fn stringify_pdf(
    ...
    let pdf_path =
        crate::convert::convert_document_to_document(db, &docx_path, "docx", "pdf").await?;
...
pub fn parse_pdf(path: &str) -> Result<Json, String> {
    ...
    let full_text =
        if extracted_text.contains("\u{FFFD}") || has_latin1_krakozyabry(&extracted_text) {
