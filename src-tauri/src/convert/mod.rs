...
use crate::convert::pdf::{parse_pdf, stringify_pdf};
...
use crate::convert::docx::{parse_docx, stringify_docx};
use crate::convert::odt::{parse_odt, stringify_odt};
use crate::convert::xlsx::{parse_xlsx, stringify_xlsx};

use local_utils::convert_with_soffice_explicit;

use crate::convert::local_utils::{
    convert_docx_to_rtf, xml_to_html_via_soffice, xml_to_rtf_via_soffice,
};
use crate::db;
...
pub async fn convert(
    ...
    match (from_type, to_type) {
        ...
        (ContentType::Text, ContentType::Text) => convert_text_to_text(db, path, from, to).await,
        ...
        // Text → Audio
        (ContentType::Text, ContentType::Audio) => convert_text_to_audio(path, from, to).await,
        ...
        // Image → Image
        (ContentType::Image, ContentType::Image) => convert_image_to_image(path, from, to).await,

        // Image → Text
        (ContentType::Image, ContentType::Text) => convert_image_to_text(path, from, to).await,
        (ContentType::Image, ContentType::Document) => {
            convert_image_to_document(db, path, from, to).await
        }

        // Audio → Audio
        (ContentType::Audio, ContentType::Audio) => convert_audio_to_audio(path, from, to).await,

        // Audio → Text
        (ContentType::Audio, ContentType::Text) => convert_audio_to_text(db, path, from, to).await,
        ...
        // Video → Video
        (ContentType::Video, ContentType::Video) => convert_video_to_video(path, from, to).await,

        // Video → Audio
        (ContentType::Video, ContentType::Audio) => convert_video_to_audio(path, from, to).await,
        // Video → Text (извлекаем аудио, потом распознаем)
        (ContentType::Video, ContentType::Text) => convert_video_to_text(db, path, from, to).await,
...
async fn convert_document_to_text(
    ...
    if to == "rtf" {
        ...
        if from == "pdf" {
            ...
            let temp_text_path =
                std::env::temp_dir().join(format!("temp_{}.txt", uuid::Uuid::new_v4().simple()));
            ...
            let docx_path =
                convert_text_to_document(db, temp_text_path.to_str().unwrap(), "txt", "docx")
                    .await?;
...
async fn convert_image_to_document(
    db: &DatabaseConnection,
    path: &str,
    from: &str,
    to: &str,
...
pub async fn convert_document_to_document(
    ...
    match (from, to) {
        ...
        ("docx", "xlsx") => {
            ...
            let doc =
                office_oxide::Document::open(path).map_err(|e| format!("Open DOCX: {}", e))?;
            doc.save_as(&out)
        ...
        ("xlsx", "docx") => {
            let out = out_path("docx")?;
            let doc =
                office_oxide::Document::open(path).map_err(|e| format!("Open XLSX: {}", e))?;
            doc.save_as(&out)
...
use image::codecs::{
    farbfeld::FarbfeldEncoder, hdr::HdrEncoder, ico::IcoEncoder, openexr::OpenExrEncoder,
};
use image::{
    DynamicImage, EncodableLayout, ExtendedColorType, ImageEncoder, ImageFormat, ImageReader,
};
        ...
        Ok(reader) => {
            if from == "pnm" || path.to_lowercase().ends_with(".pnm") {
                match reader.with_guessed_format() {
                    Ok(r) => match r.decode() {
                        Ok(img) => img,
                        Err(e) => match ImageReader::open(path) {
                    ...
                    Err(_) => match ImageReader::open(path) {
                        Ok(r) => r
                            .decode()
...
async fn convert_audio_to_document(
    ...
    // 2. Читаем текст
    let text =
        std::fs::read_to_string(&text_path).map_err(|e| format!("Cannot read text file: {}", e))?;
...
pub fn save_to_app_dir(
    ...
    let output_path = get_app_dir_path_with_hash(original_path, to, hash)?;
...
pub async fn convert_file(
    ...
    from: String,
    to: String,
    #[allow(nonstandard_style)] fromType: String,
    #[allow(nonstandard_style)] toType: String,
    enable_cache: bool,
    ...
    let original_name = output_path_buf
        .file_stem()
        .and_then(|s| s.to_str())
    ...
    if output_path_buf != new_output_path {
        ...
        println!("File moved: {:?} -> {:?}", output_path_buf, new_output_path);
    }
...
mod tests {
        ...
        Fut: std::future::Future<Output = Result<String, String>>,
    {
        ...
        if !errors.is_empty() {
            ...
            panic!(
                "\n❌ {} conversions failed (see error summary above)",
                errors.len()
            );
        }
    }
    ...
    const AUDIO_FORMATS: &[&str] = &[
        "mp3", "wav", "aac", "flac", "ogg", "opus", "wma", "m4a", "aiff", "ac3", "eac3", "tta",
        "wv", "voc", "adx", "aptx", "sbc", "caf", "w64",
    ];

    const IMAGE_FORMATS: &[&str] = &[
        "jpg", "png", "webp", "avif", "gif", "bmp", "tiff", "ico", "qoi", "tga", "exr", "hdr",
        "pnm", "ff",
    ];

    const VIDEO_FORMATS: &[&str] = &[
        "mp4", "mov", "avi", "mkv", "webm", "wmv", "flv", "3gp", "m4v", "ts", "vob", "mpg", "nut",
    ];
    ...
    const DOCUMENT_FORMATS: &[&str] = &["pdf", "docx", "odt", "xlsx"];
    ...
    mod document_to_document {
        ...
        async fn test_docx_to_all_document_formats() {
            test_conversion(
                "docx",
                super::DOCUMENT_FORMATS,
                |db, path, from, to| async move {
                    convert_document_to_document(&db, &path, &from, &to).await
                },
                None,
            )
    ...
    mod image_to_text {
        ...
        async fn test_jpg_to_all_text_formats() {
            ...
            test_conversion(
                "jpg",
                super::TEXT_FORMATS,
                |_db, path, from, to| async move { convert_image_to_text(&path, &from, &to).await },
                Some(db),
            )
            .await;
        }
    ...
    mod text_to_audio {
        ...
        async fn test_txt_to_all_audio_formats() {
            ...
            test_conversion(
                "txt",
                super::AUDIO_FORMATS,
                |_db, path, from, to| async move { convert_text_to_audio(&path, &from, &to).await },
                Some(db),
            )
            .await;
        }
    ...
    mod audio_to_text {
        ...
        #[tokio::test]
        async fn test_mp3_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("mp3", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_wav_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("wav", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_aac_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("aac", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_flac_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("flac", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_ogg_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("ogg", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_opus_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("opus", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_wma_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("wma", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_m4a_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("m4a", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_aiff_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("aiff", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_ac3_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("ac3", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_eac3_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("eac3", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_tta_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("tta", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_wv_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("wv", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_voc_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("voc", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_adx_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("adx", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_aptx_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("aptx", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_sbc_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("sbc", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_caf_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("caf", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }

        #[tokio::test]
        async fn test_w64_to_all_text_formats() {
            let db = create_test_db().await.unwrap();
            test_conversion("w64", super::TEXT_FORMATS, |db, path, from, to| async move {
                convert_audio_to_text(&db, &path, &from, &to).await
            }, Some(db)).await;
        }
    }
